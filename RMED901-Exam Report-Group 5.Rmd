...
```
**#Tidying data 2.5:** Check the missing data and number of the missing data
```{r}
...
**#3.1:** Remove unnecessary columns from your dataframe: acinar, train, amp, pdstent. Use subset() fuction to delete column by name
```{r}
...
**#3.2:** Change the variable types(Make necesary changes in variable types)
...
# 5. Does the outcome depend on the site where the procedure was performed
```{r}
ggplot(Fulldataset,
...
# 6. Does the outcome depend on the gender of the patient?
```{r}
ggplot(Fulldataset,
...
# 7. Does the outcome depend on whether there was a trainee present during the procedure?
```{r}
...
# 8. According to the data, was the indomethacin reducing the risk of pancreatitis?
```{r}
Fulldataset %>% 
...
```
#Looking at the data distribution first. We made the model for indometachin vs risk of pancreatitis. 
